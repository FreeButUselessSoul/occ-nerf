...
def model_render_image(c2w, rays_cam, t_vals, near, far, H, W, fxfy, model, perturb_t, sigma_noise_std,
                       args, rgb_act_fn,features=None,proj_mats=None,pixel_i=None,cost_volume_cnn=None,cost_volume=None):
    ...
    result = {
            ...
            'weight': render_result['weight'] * torch.mean(cost_volume,-1),
        }
...
##
        for ii,rays_dir_rows in enumerate(rays_dir_cam_split_rows):
            ...
            render_result = model_render_image(c2w, rays_dir_rows, t_vals, scene_train.near, scene_train.far,
                                               ...
                                               model, False, 0.0, args, rgb_act_fn,cost_volume=cost_volume_split_rows,
                                               )
...
def train_one_epoch(scene_train, optimizer_nerf, optimizer_focal, optimizer_pose, model, focal_net, pose_param_net, cost_volume_cnn,
...
##
        ...
        render_result = model_render_image(c2w, ray_selected_cam, t_vals, scene_train.near, scene_train.far,
                                        ...
                                        features,torch.stack(proj_mats),pixel_i,cost_volume_cnn)  # (N_select_rows, N_select_cols, 3)
...
def main(args):
    ...
    '''Create Folders'''
    exp_root_dir = Path(os.path.join('./logs/homo', args.scene_name))
    ...
    shutil.copy('./tasks/homo/train_homo.py', experiment_dir)
    ...
    cost_volume_cnn = torch.nn.Sequential(
        # conv(640,256,3),
        # conv(256,128,3),
        # conv(128,64,3),
    ...
    scheduler_nerf = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_nerf, args.epoch,1e-4)
    scheduler_focal = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_focal, int(args.epoch*0.8),1e-4)
    scheduler_pose = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_pose, int(args.epoch*0.8),1e-4)
    ...
    for epoch_i in tqdm(range(args.epoch), desc='epochs'):
        ...
        logger.info('{:6d} ep: Train: L2 loss: {:.4f}, Colume:{:.3f}, PSNR: {:.3f}'.format(epoch_i, train_L2_loss,train_cost_volume_loss, train_psnr))
        tqdm.write('{:6d} ep: Train: L2 loss: {:.4f}, Colume:{:.3f}, PSNR: {:.3f}'.format(epoch_i, train_L2_loss,train_cost_volume_loss, train_psnr))
