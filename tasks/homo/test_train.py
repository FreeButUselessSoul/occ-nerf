...
def main(args):
    ...
    '''Create Folders'''
    temp_scene_name = args.scene_name.replace('/','_')+'_nerfmm'
    ...
    for i in range(scene_train.N_imgs):
        img_gt = scene_train.imgs[i]
        img_pred = imgs[i].to(img_gt.device)
        psnrs += [piq.psnr(img_gt, img_pred).item()]
        lpips += [piq.LPIPS(reduction='none')(img_pred, img_gt)]
        ssims += [piq.ssim(img_pred, img_gt)]

    print(f'Mean PSNR : {torch.mean(psnrs):.2f}')
    print(f'Mean SSIM : {torch.mean(ssims):.2f}')
    print(f'Mean LPIPS : {torch.mean(lpips):.2f}')
    return
