...
def volume_sampling_ndc(c2w, ray_dir_cam, t_vals, near, far, H, W, focal, perturb_t, use_disp=False, output_raw=False):
    ...
    if perturb_t:
        if use_disp:
            t_noise = torch.rand((ray_H, ray_W, N_sam), device=c2w.device, dtype=torch.float32)  # (H, W, N_sam)
            t_noise = t_noise * (far - near) / N_sam
            t_vals_noisy = 1/(torch.clamp(1/t_vals.view(1, 1, N_sam) + t_noise,1e-15))  # (H, W, N_sam)
        else:
            # add some noise to each each z_val
            t_noise = torch.rand((ray_H, ray_W, N_sam), device=c2w.device, dtype=torch.float32)  # (H, W, N_sam)
            t_noise = t_noise * (far - near) / N_sam
    ...
    if output_raw:
        return sample_pos, ray_ori_world, ray_dir_world, t_vals_noisy,non_NDC_info
    return sample_pos, ray_ori_world, ray_dir_world, t_vals_noisy  # (H, W, N_sample, 3), (3, ), (H, W, 3), (H, W, N_sam)


def volume_rendering(rgb_density, t_vals, sigma_noise_std, rgb_act_fn, density2=None):
    """
    ...
    if density2 is not None:
        # threshold = num_sample//4
        # mask = torch.int(depth_map)<threshold
        # arr = torch.arange(num_sample).to(depth_map.device)[None,None,:].repeat(depth_map.shape[0],depth_map.shape[1],1)
        # arr = t_vals[None,None,:].repeat(depth_map.shape[0],depth_map.shape[1],1)
        if sigma_noise_std > 0.0:
            density2 = density2 + sigma_noise  # (H, W, N_sample)

        density2 = density2.relu()  # (H, W, N_sample)
        alpha_2 = 1 - torch.exp(-1.0 * density2 * dists)  # (H, W, N_sample)
        # 1. We expand the exp(a+b) to exp(a) * exp(b) for the accumulated transmittance computing.
        # 2. For the space at the boundary far to camera, the alpha is constant 1.0 and the transmittance at the far boundary
        # is useless. For the space at the boundary near to camera, we manually set the transmittance to 1.0, which means
        # 100% transparent. The torch.roll() operation simply discards the transmittance at the far boundary.
        acc_transmittance_2 = torch.cumprod(1.0 - alpha_2 + 1e-10, dim=2)  # (H, W, N_sample)
        acc_transmittance_2 = torch.roll(acc_transmittance_2, shifts=1, dims=2)  # (H, W, N_sample)
        acc_transmittance_2[:, :, 0] = 1.0  # (H, W, N_sample)
        weight_2 = acc_transmittance_2 * alpha_2  # (H, W, N_sample)
        # (H, W, N_sample, 1) * (H, W, N_sample, 3) = (H, W, N_sample, 3) -> (H, W, 3)
        rgb_rendered_2 = torch.sum(weight_2.unsqueeze(3) * rgb, dim=2)
