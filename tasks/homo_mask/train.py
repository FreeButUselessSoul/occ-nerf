...
from models.nerf_mask import OfficialNerf
...
def model_render_image(c2w, rays_cam, t_vals, near, far, H, W, fxfy, model, perturb_t, sigma_noise_std,
    ...
    temp_result = model(pos_enc, dir_enc_)  # (H, W, N_sample, 4)
    rgb_density = temp_result[...,:4]
    mask = temp_result[...,-1]
    val_rank = 1-t_vals_noisy;val_rank = F.normalize(val_rank,dim=-1)
    ismasked = torch.sum(mask * val_rank, -1)
...
## 
        ...
        for j in others[:6]:
            ...
            features.append(scene_train.features[j].to(my_devices))
...
##
        ...
        rendered_mask = torch.cat(rendered_mask, dim=0).unsqueeze(0)
        ...
        rendered_mask_list.append(rendered_mask.cpu().numpy())
    ...
    disp_mask = rendered_mask_list[rand_num]
    ...
    writer.add_image('mask/eval',disp_mask,global_step=epoch_i)
...
##
        ...
        # else:
        # masked_L2_loss = torch.mean(torch.mean((rgb_rendered - img_selected) ** 2,-1) * mask) # mask=1:bg, =0:fg
        L2_loss = F.mse_loss(rgb_rendered, img_selected)
        depth_loss = -0.1*torch.mean(cov(torch.stack([render_result['depth_map'].flatten(0,1), mask.flatten(0,1)])))

        tot_loss = L2_loss +cost_volume_loss*50 + depth_loss
        tot_loss.backward()
        optimizer_nerf.step()
        ...
        depth_loss_epoch.append(depth_loss.item())
    ...
    depth_loss_epoch_mean = np.mean(depth_loss_epoch)
    mean_losses = {
        'L2': L2_loss_epoch_mean,
        'cost_volume': cost_volume_loss_epoch_mean,
        'depth' : depth_loss_epoch_mean,
    }
    return mean_losses
...
def main(args):
    ...
    scheduler_focal = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_focal, int(args.epoch*0.8),min(1e-5,args.focal_lr))
    scheduler_pose = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_pose, int(args.epoch*0.8),min(1e-5,args.pose_lr))
    ...
    for epoch_i in tqdm(range(args.epoch), desc='epochs'):
        ...
        train_epoch_losses = train_one_epoch(scene_train, optimizer_nerf, optimizer_focal, optimizer_pose, optimizer_volume,
                                             model, focal_net, pose_param_net, cost_volume_cnn, my_devices, args, rgb_act_fn, epoch_i)
        train_L2_loss = train_epoch_losses['L2']
        train_cost_volume_loss = train_epoch_losses['cost_volume']
        train_depth_loss = train_epoch_losses['depth']
        scheduler_nerf.step()
        ...
        logger.info('{:6d} ep: Train: L2 loss: {:.4f}, Colume:{:.3f}, D:{:.3f}, PSNR: {:.3f}'.format(epoch_i, train_L2_loss,train_cost_volume_loss,train_depth_loss, train_psnr))
        tqdm.write('{:6d} ep: Train: L2 loss: {:.4f}, Colume:{:.3f}, D:{:.3f}, PSNR: {:.3f}'.format(epoch_i, train_L2_loss,train_cost_volume_loss,train_depth_loss, train_psnr))

        if epoch_i % args.eval_interval == 0 and epoch_i > 0:
            with torch.no_grad():
                eval_one_epoch(eval_c2ws, scene_train, model, focal_net, pose_param_net,cost_volume_cnn, my_devices, args, epoch_i, writer, rgb_act_fn)
                ...
                save_checkpoint(epoch_i, cost_volume_cnn, optimizer_volume, exp_root_dir, ckpt_name='cost_volume_cnn')
    return
